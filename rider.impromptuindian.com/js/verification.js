...
async function confirmUpload() {
    ...
    try {
        const response = await ImpromptuIndianApi.fetch('/api/rider/verification/upload', {
            method: 'POST',
            body: formData
        });
    ...
    try {
        ...
        const uploadPromises = Object.keys(documents).map(async (docType) => {
            ...
            if (doc.file) { // Only upload if there is a new file
                ...
                const resp = await ImpromptuIndianApi.fetch('/api/rider/verification/upload', {
                    method: 'POST',
                    body: formData
                });
        ...
        const submitResp = await ImpromptuIndianApi.fetch('/api/rider/verification/submit', {
            method: 'POST',
