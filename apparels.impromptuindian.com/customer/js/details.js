...
function renderOrderDetails(order) {
    ...
    if (hasRiderInfo) {
        ...
        // Find the address card container - try multiple methods for reliability
        let addressCard = null;
        const addressBox = document.getElementById('address-box');
        if (addressBox) {
            // Try to find parent card with bg-[#0b1220] class
            addressCard = addressBox.closest('.bg-\\[\\#0b1220\\]') || 
                         addressBox.closest('[class*="bg-"]') ||
                         addressBox.parentElement;
        }
        
        // Insert delivery info card after address card, or append to left column
        if (addressCard && addressCard.parentNode) {
            addressCard.parentNode.insertBefore(deliveryDiv, addressCard.nextSibling);
        } else {
            // Fallback: Find the left column (lg:col-span-2) and append
            const leftColumn = document.querySelector('.lg\\:col-span-2') || 
                              document.querySelector('[class*="col-span-2"]');
            if (leftColumn) {
                leftColumn.appendChild(deliveryDiv);
            } else {
                console.error('Could not find container for delivery info card');
            }
        }
        ...
        if ((order.delivery_method === 'inhouse' || order.rider_name) && order.rider_name) {
            deliveryInfoHtml += `
                <div class="bg-[#111827] p-4 rounded-lg border border-gray-700">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <span class="text-sm text-gray-400 uppercase font-semibold">In-House Rider</span>
                    </div>
                    <div class="space-y-3">
                        <div>
                            <span class="block text-gray-500 text-xs uppercase mb-1">Rider Name</span>
                            <span class="text-white font-semibold text-base">${order.rider_name}</span>
                        </div>
                        ${order.rider_phone ? `
                        <div>
                            <span class="block text-gray-500 text-xs uppercase mb-1">Contact Number</span>
                            <a href="tel:${order.rider_phone}" class="text-white font-semibold text-base hover:text-green-400 transition-colors flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <span class="text-green-400 font-semibold text-base flex items-center gap-2">
            ...
            deliveryInfoHtml += `
                <div class="bg-[#111827] p-4 rounded-lg border border-gray-700">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <span class="text-sm text-gray-400 uppercase font-semibold">Platform Rider</span>
                    </div>
                    <div>
            ...
            // Fallback: Show any available rider info even if delivery_method is not set
            if (order.rider_name || order.rider_phone || order.expected_delivery) {
                deliveryInfoHtml += `
                    <div class="bg-[#111827] p-4 rounded-lg border border-gray-700">
                        <div class="flex items-center gap-2 mb-3">
                            <i data-lucide="user" class="w-4 h-4 text-blue-400"></i>
                            <span class="text-sm text-gray-400 uppercase font-semibold">Delivery Rider</span>
                        </div>
                        <div class="space-y-3">
                            ${order.rider_name ? `
                            <div>
        ...
        // Remove delivery info card if order has no delivery info
