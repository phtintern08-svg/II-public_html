    ...
    async getCurrentLocation() {
        ...
        return new Promise((resolve, reject) => {
            ...
            const handleSuccess = async (pos) => {
                ...
                console.log(`GPS Lock acquired: ${latitude}, ${longitude}. Fetching address details...`);
                try {
                    const addressData = await this.reverseGeocodeMappls(latitude, longitude);
                    resolve({
                    ...
                    // Still resolve with coords even if reverse geocode fails, 
                    // since coords are vital for the backend.
                    resolve({
                        coords: { latitude, longitude },
                        address: null,
            ...
            const handleError = (err) => {
                ...
                // If high accuracy timed out, try again with low accuracy
                if (err.code === 3 && geoOptions.enableHighAccuracy) {
                    console.warn("High accuracy timed out, trying standard accuracy...");
                    geoOptions.enableHighAccuracy = false;
                    navigator.geolocation.getCurrentPosition(handleSuccess, (err2) => {
                        let msg = "Location error: " + err2.message;
